...
**Estimación Global:** 11 Story Points (desglosados en 5 PBIs)  
...
- **Entonces** el sistema responde inmediatamente con código `HTTP 429 Too Many Requests`, cuerpo tipado en `OperationEnvelope` indicando el tiempo de espera recomendado, sin consumir cuota del motor LLM.
...
- **Cuando** el backend orquesta la respuesta mediante Server-Sent Events (SSE).
...
- **Entonces** el hook `.githooks/pre-commit` valida automáticamente tipos y linter sobre los archivos staged, rechazando la operación si existen errores; complementado con la ejecución programada de `PruneTelemetryUseCase`.
