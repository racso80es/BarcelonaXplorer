...
**Entorno:** Git 2.x, Bash Shell, Linux Cron / Next.js Instrumentation, Prisma ORM / MySQL  
...
- **Naturaleza:** Instalación de una aduana local `pre-commit` mediante Git Hooks puros (`core.hooksPath = .githooks`), e implementación de una tarea de mantenimiento programada para ejecutar automáticamente la purga de logs antiguos de MySQL mediante `PruneTelemetryUseCase`.
- **Entorno:** `.githooks/pre-commit`, `scripts/install-hooks.sh`, `src/features/telemetry/prune-telemetry.use-case.ts`.
- **Entropía Asimilada (Filtros A, B y C):**
  - *Filtro A (Rigor Técnico):* El hook `pre-commit` ejecuta de manera incremental y ultrarrápida la comprobación de tipos (`tsc --noEmit`) y el linter AST sobre los archivos en staged, abortando el commit local si detecta `any` o errores sintácticos.
...
- [ ] **CA-1 (Hook Pre-commit Local):** Creación del directorio `.githooks/` conteniendo `pre-commit` ejecutable que valida `tsc --noEmit` y `npm run lint` sobre el código antes de permitir el commit.
- [ ] **CA-2 (Script de Inicialización de Hooks):** Script `scripts/setup-hooks.sh` que configura automáticamente `git config core.hooksPath .githooks` y asigna permisos de ejecución `chmod +x`.
- [ ] **CA-3 (Endpoint o Tarea Programada de Purga):** Exposición de una ruta protegida interna `/api/telemetry/prune` o integración en `instrumentation.ts` invocando `PruneTelemetryUseCase` de forma segura.
- [ ] **CA-4 (Verificación de Oráculos):** Pruebas unitarias colocadas en `src/features/telemetry/prune-telemetry.test.ts` verificando que la purga automática borra únicamente registros caducados según las políticas de retención.
